...
struct ContentView: View {
    ...
    init() {
        ...
        // Tools klasörünün bundle içinde olup olmadığını kontrol et
        if let bundlePath = Bundle.main.resourcePath {
            let toolsPath = bundlePath + "/Tools"
            let fileManager = FileManager.default
            if fileManager.fileExists(atPath: toolsPath) {
                print("✅ Tools klasörü bundle içinde bulundu: \(toolsPath)")
                let ytDlpPath = toolsPath + "/yt-dlp"
                let aria2cPath = toolsPath + "/aria2c"
                let ffmpegPath = toolsPath + "/ffmpeg"
                print("yt-dlp exists: \(fileManager.fileExists(atPath: ytDlpPath))")
                print("aria2c exists: \(fileManager.fileExists(atPath: aria2cPath))")
                print("ffmpeg exists: \(fileManager.fileExists(atPath: ffmpegPath))")
            } else {
                print("❌ Tools klasörü bundle içinde bulunamadı!")
            }
        }
    }
    ...
    private func findTool(named name: String) -> String? {
        // Önce bundle içindeki araçları kontrol et (Resources klasöründe)
        if let bundlePath = Bundle.main.resourcePath {
            let toolPath = bundlePath + "/" + name
            if FileManager.default.isExecutableFile(atPath: toolPath) {
                // Bundle içindeki aracı geçici dizine kopyala (security için)
                let tempDir = NSTemporaryDirectory()
                let tempPath = tempDir + name + "_bundle"
                
                // Eski geçici dosyayı sil
                try? FileManager.default.removeItem(atPath: tempPath)
                
                do {
                    try FileManager.default.copyItem(atPath: toolPath, toPath: tempPath)
                    try FileManager.default.setAttributes([.posixPermissions: 0o755], ofItemAtPath: tempPath)
                    print("✅ Bundle araç kopyalandı: \(name) -> \(tempPath)")
                    return tempPath
                } catch {
                    print("❌ Bundle araç kopyalama hatası: \(error)")
                }
            }
        }
        
        // Bundle'da yoksa sistem araçlarını kontrol et
        let candidates = ["/usr/local/bin/\(name)", "/opt/homebrew/bin/\(name)"]
        for path in candidates where FileManager.default.isExecutableFile(atPath: path) { 
            print("✅ Sistem aracı bulundu: \(path)")
            return path 
        }
        ...
        for dir in paths { 
            ...
            if FileManager.default.isExecutableFile(atPath: full) { 
                print("✅ PATH aracı bulundu: \(full)")
                return full 
            }
        }
        print("❌ Araç bulunamadı: \(name)")
        return nil
    }
